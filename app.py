...
def my_form_post():
    ...
    character_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[0].values())[0]]
    character_table = ItemTable(character_items, border=True)
    one_class = list(store[1].keys())[0]
    one_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[1].values())[0]]
    one_table = ItemTable(one_items, border=True)
    two_class = list(store[2].keys())[0]
    two_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[2].values())[0]]
    two_table = ItemTable(two_items, border=True)
    three_class = list(store[3].keys())[0]
    three_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[3].values())[0]]
    three_table = ItemTable(three_items, border=True)
    four_class = list(store[4].keys())[0]
    four_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[4].values())[0]]
    four_table = ItemTable(four_items, border=True)
    five_class = list(store[5].keys())[0]
    five_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[5].values())[0]]
    five_table = ItemTable(five_items, border=True)
    six_class = list(store[6].keys())[0]
    six_items = [Item(item[0], item[1], item[2], item[3]) for item in list(store[6].values())[0]]
